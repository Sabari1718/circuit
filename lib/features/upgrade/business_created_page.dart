...
class _BusinessCreatedPageState extends State<BusinessCreatedPage> {
  ...
  final List<Map<String, dynamic>> _regTypes = [
    {"title": "Propagator", "icon": Icons.hub_rounded, "color": const Color(0xFF8B5CF6)},
    {"title": "Partner", "icon": Icons.handshake_rounded, "color": const Color(0xFF3B82F6)},
    {"title": "Create Supplier", "icon": Icons.inventory_2_rounded, "color": const Color(0xFFE11D48)},
  ];
  ...
  void _openDetails(BuildContext context, BusinessUser biz) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => BusinessRegistrationOverviewPage(business: biz)),
    );
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (businesses.any((b) => b.registrationType == 'Propagator' || b.registrationType == null)) ...[
                          ...
                          _buildYourBusinessesHeader("Your Businesses", businesses.where((b) => b.registrationType == 'Propagator' || b.registrationType == null).length),
                          const SizedBox(height: 20),
                          ...businesses.where((b) => b.registrationType == 'Propagator' || b.registrationType == null).map((biz) => _buildBusinessCard(context, biz, isDesktop)).toList(),
                        ],

                        if (businesses.any((b) => b.registrationType == 'Partner')) ...[
                          const SizedBox(height: 32),
                          _buildYourBusinessesHeader("Partner Businesses", businesses.where((b) => b.registrationType == 'Partner').length),
                          const SizedBox(height: 20),
                          ...businesses.where((b) => b.registrationType == 'Partner').map((biz) => _buildBusinessCard(context, biz, isDesktop)).toList(),
                        ],

                        if (businesses.any((b) => b.registrationType == 'Supplier')) ...[
                          const SizedBox(height: 32),
                          _buildYourBusinessesHeader("Supplier Businesses", businesses.where((b) => b.registrationType == 'Supplier').length),
                          const SizedBox(height: 20),
                          ...businesses.where((b) => b.registrationType == 'Supplier').map((biz) => _buildBusinessCard(context, biz, isDesktop)).toList(),
                        ],
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          _topIcon(Icons.notifications_none_outlined, badge: "4"),
  ...
  Widget _buildUserAvatar() {
    return Container(
      ...
      child: const Center(child: Text("S", style: TextStyle(color: Color(0xFF94A3B8), fontWeight: FontWeight.bold, fontSize: 13))),
    );
  }

  Widget _buildHeaderSection(BuildContext context, bool isDesktop) {
    final biz = BusinessUserStore().businesses.isNotEmpty ? BusinessUserStore().businesses.first : null;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Color(0xFF3B82F6), shape: BoxShape.circle), child: const Icon(Icons.add, color: Colors.white, size: 14)),
                        const SizedBox(width: 12),
                        Flexible(child: Text("Welcome back, $name! 👋", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800))),
                      ],
                    ),
                      ...
                      children: [
                        const Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Flexible(child: Text("Your business will get a unique 10-digit ID: ", style: TextStyle(color: Color(0xFF64748B), fontSize: 13, fontWeight: FontWeight.w500))),
                          ],
                        ),
              ...
              if (isDesktop)
                Padding(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFE11D48), elevation: 0, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                    child: const Text("# New Registration", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12)),
                  ),
                ),
            ],
          );
  ...
  Widget _buildVerificationBanner() {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF9299D6).withOpacity(0.9), borderRadius: BorderRadius.circular(12)),
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: Colors.yellow[600], shape: BoxShape.circle), child: const Icon(Icons.history, color: Colors.white, size: 20)),
              const SizedBox(width: 12),
              const Flexible(child: Text("Verification in Progress", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16))),
            ],
          ),
          const SizedBox(height: 12),
          const Text(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              icon: Icon(_showRegTypeSelection ? Icons.close : Icons.add_circle_outline, color: Colors.white, size: 20),
              label: Text(
  ...
  Widget _buildRegistrationTypeSelector(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Select Registration Type", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14)),
          const SizedBox(height: 16),
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: _regTypes.map((type) => DropdownMenuItem<String>(
                  value: type['title'],
                  child: Row(
                    children: [
                      Icon(type['icon'], color: type['color'], size: 20),
                      const SizedBox(width: 12),
                      Text(type['title'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                    ],
                  ),
                )).toList(),
                onChanged: (val) {
  ...
  Widget _buildYourBusinessesHeader(String label, int count) {
    return Row(
      children: [
        const Icon(Icons.business_center_rounded, color: Color(0xFF3B82F6), size: 20),
        const SizedBox(width: 8),
        Text("$label ($count)", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
      ],
    );
  }
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFE11D48), borderRadius: BorderRadius.circular(6)), child: const Text("Active", style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800))),
                    const SizedBox(width: 12),
                    const Icon(Icons.badge_outlined, size: 14, color: Color(0xFF64748B)),
                    const SizedBox(width: 4),
                    Expanded(child: Text("ID: ${biz.id}", style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 12))),
                    IconButton(icon: const Icon(Icons.more_vert_rounded, color: Color(0xFF64748B), size: 18), onPressed: () {}),
                  ],
                ),
              ),
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        Container(width: 44, height: 44, decoration: const BoxDecoration(color: Color(0xFFE11D48), shape: BoxShape.circle), child: const Center(child: Icon(Icons.business_rounded, color: Colors.white, size: 22))),
                        const SizedBox(width: 16),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(biz.businessName, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15)),
                              const SizedBox(height: 6),
                              Wrap(spacing: 8, runSpacing: 8, children: biz.businessTypes.map((t) => _buildTypeChip(t)).toList()),
                            ],
                          ),
                        ),
                      ],
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("Added: ${_formatDate(biz.createdDate)}", style: const TextStyle(fontSize: 11, color: Color(0xFF94A3B8), fontWeight: FontWeight.w600)),
                        OutlinedButton(
                          onPressed: () => _openDetails(context, biz),
                          style: OutlinedButton.styleFrom(side: const BorderSide(color: Color(0xFFE11D48)), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                          child: const Text("View Details", style: TextStyle(color: Color(0xFFE11D48), fontWeight: FontWeight.w800, fontSize: 11)),
                        ),
                      ],
                    ),
                  ],
  ...
  Widget _buildDetailsRows(BusinessUser biz, bool isSmall) {
    ...
    final List<Widget> items = [
      _buildInfoLine(Icons.location_on_rounded, "${biz.doorNumber}, ${biz.streetName}, ${biz.area}"),
      _buildInfoLine(Icons.email_rounded, biz.email),
      _buildInfoLine(Icons.phone_rounded, biz.phone),
      _buildInfoLine(Icons.history_rounded, "Est. ${biz.yearOfEstablishment} • ${biz.employeeRange} employees"),
      _buildInfoLine(Icons.vpn_key_rounded, "PAN: ${biz.panNumber}"),
      _buildInfoLine(Icons.receipt_long_rounded, "GST: ${biz.gstNumber}"),
      if (isPartner) _buildInfoLine(Icons.groups_rounded, "Partners: ${biz.partnerCount}"),
    ];
    ...
    return Column(
      children: [
        Row(children: [Expanded(child: items[0]), const SizedBox(width: 20), Expanded(child: items[1])]),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: items[2]), const SizedBox(width: 20), Expanded(child: items[3])]),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: items[4]), const SizedBox(width: 20), Expanded(child: items[5])]),
        if (isPartner) ...[
          const SizedBox(height: 12),
          Row(children: [Expanded(child: items[6]), const SizedBox(width: 20), const Spacer()]),
        ]
      ],
    );
  }

  Widget _buildInfoLine(IconData icon, String text) => Row(children: [Icon(icon, size: 13, color: const Color(0xFF94A3B8)), const SizedBox(width: 8), Expanded(child: Text(text, style: const TextStyle(fontSize: 12, color: Color(0xFF475569), fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis))]);
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                Container(width: 48, height: 28, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4)), child: const Center(child: Text("90×25", style: TextStyle(color: Colors.black, fontSize: 8, fontWeight: FontWeight.bold)))),
                const Spacer(),
  ...
  Widget _topIcon(IconData icon, {String? badge}) => Stack(children: [IconButton(icon: Icon(icon, size: 22), onPressed: () {}), if (badge != null) Positioned(right: 12, top: 12, child: Container(padding: const EdgeInsets.all(2), decoration: BoxDecoration(color: const Color(0xFFE11D48), borderRadius: BorderRadius.circular(6)), constraints: const BoxConstraints(minWidth: 12, minHeight: 12), child: Text(badge, style: const TextStyle(color: Colors.white, fontSize: 7, fontWeight: FontWeight.bold), textAlign: TextAlign.center)))]);
}