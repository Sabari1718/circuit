...
class _BusinessStep3PageState extends State<BusinessStep3Page> {
  ...
  Future<void> _submitRegistration() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // Trigger background refresh APIs as per the original flow
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      const Text("Business User", style: TextStyle(color: Color(0xFF2563EB), fontWeight: FontWeight.bold)),
                    ],
                  ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildResponsiveRow(isMobile, [
                          Expanded(child: _buildTextField("Bank Account Number", "Enter Bank Account Number", _bankAccountController, isRequired: true)),
                          const SizedBox(width: 24),
                          Expanded(child: _buildTextField("Confirm Account Number", "Confirm Account Number", _confirmAccountController, isRequired: true)),
                        ]),
                        const SizedBox(height: 24),
                        _buildResponsiveRow(isMobile, [
                          Expanded(
                            child: _buildDropdown(
                              ...
                              items: ["Bank Passbook (Front Page)", "Bank Statement (Last 3 Months)", "Cancelled Cheque"],
                        ...
                        _buildResponsiveRow(isMobile, [
                          Expanded(
                            child: _buildDropdown(
                              ...
                              items: ["Factory", "Warehouse / Godown", "Office", "Branch", "Shop", "Other"],
                              onChanged: (val) {
                                setState(() {
                          ...
                          if (_selectedAddressType == 'Other') ...[
                            const SizedBox(width: 24),
                            Expanded(
                              child: _buildTextField("Custom Address Type", "e.g. Retail Shop", _customAddressTypeController, isRequired: true),
                            ),
                          ],
                          const SizedBox(width: 24),
                          Expanded(
                        ...
                        _buildResponsiveRow(isMobile, [
                          Expanded(
                            child: _buildDropdown(
                              ...
                              items: ["Aadhaar Card", "Passport", "Voter ID", "Driving Licence", "Utility Bill"],
                              onChanged: (val) => setState(() => _selectedProofType = val),
                              isRequired: true,
                            ),
                          ),
                          const SizedBox(width: 24),
  ...
  Widget _buildDynamicAddressFields(bool isMobile) {
    ...
    return Column(
      ...
      children: [
        _buildResponsiveRow(isMobile, [
          if (isOffice) ...[
            Expanded(child: _buildTextField(nameLabel, "Office/Building Name", _nameController, isRequired: true)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField(noLabel, "Office No", _noController, isRequired: true)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField("Floor", "e.g. 3rd Floor", _floorController, isRequired: false)),
          ] else if (isOther) ...[
            Expanded(child: _buildTextField(noLabel, "Door No", _noController, isRequired: true)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField(nameLabel, "Building Name", _nameController, isRequired: false)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField(blockLabel, "Block", _plotBlockController, isRequired: false)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField("Floor", "Floor", _floorController, isRequired: false)),
          ] else ...[
            Expanded(child: _buildTextField(nameLabel, nameLabel, _nameController, isRequired: true)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField(noLabel, noLabel, _noController, isRequired: true)),
            const SizedBox(width: 24),
            Expanded(child: _buildTextField(blockLabel, blockLabel.split(' / ').first, _plotBlockController, isRequired: false)),
            if (showEstate) ...[
              const SizedBox(width: 24),
              Expanded(child: _buildTextField("Industrial Estate", "e.g. SIPCOT", _estateController, isRequired: false)),
            ]
          ]
        ]),
        const SizedBox(height: 24),
        _buildResponsiveRow(isMobile, [
          Expanded(child: _buildTextField("Street Name", "Street/Road Name", _streetController, isRequired: true)),
          const SizedBox(width: 24),
          Expanded(child: _buildTextField("Area / Locality", "Area Name", _areaController, isRequired: true)),
          const SizedBox(width: 24),
          Expanded(child: _buildTextField("District / City", "District", _districtController, isRequired: true)),
        ]),
        const SizedBox(height: 24),
        _buildResponsiveRow(isMobile, [
          Expanded(child: _buildTextField("State", "State", _stateController, isRequired: true)),
          const SizedBox(width: 24),
          Expanded(child: _buildTextField("Landmark", "Landmark", _landmarkController, isRequired: false)),
          const SizedBox(width: 24),
          Expanded(child: _buildTextField("Country", "India", _countryController, isRequired: false)),
        ]),
      ],
    );
  }

  Widget _buildTextField(String label, String hint, TextEditingController controller, {required bool isRequired}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        RichText(
  ...
  Widget _buildFileUpload(String label, {required bool isRequired, required String? fileName, required VoidCallback onTap}) {
            ...
            children: [
              InkWell(
                ...
                child: Container(
                  ...
                  child: const Text("Choose File", style: TextStyle(fontWeight: FontWeight.w500, color: Color(0xFF475569))),
                ),
              ),
              Expanded(
                child: Padding(
